...
for idx, x in enumerate(x_estimates):
    ...
    true_cd = Y_truth[idx + 1][-1]
    est_cd = x.mean[-1]
    est_cd_errors.append(true_cd - est_cd)
    cd_stds.append(np.sqrt(variances[-1]))
...
for idx, x in enumerate(x_estimates):
    R_ECI_to_NTW = ntw_matrix(x.mean[:3], x.mean[3:6])
...
data = {
    ...
    "est_cd_errors": [x.tolist() for x in est_cd_errors],
    "cd_stds": [x.tolist() for x in cd_stds],
...
est_cd_errors = data["est_cd_errors"]
cd_stds = data["cd_stds"]
...
# --------------- Plot Cd Error -----------------------------
cd_data = ErrorStdInput(
    x=list(T[1:]),
    y_error=[x for x in est_cd_errors],
    y_std=[x for x in cd_stds],
    x_axis_title="Time elapsed past epoch [s]",
    y_axis_title="Drag coefficient error [unitless]",
)
